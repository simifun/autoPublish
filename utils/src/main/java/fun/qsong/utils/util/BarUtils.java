...
public class BarUtils {
    ...
    public static void setTranslucentForDrawable(Activity activity, @IdRes int resId) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
...
//                            | View.SYSTEM_UI_FLAG_LAYOUT_STABLE);
        ...
        if (fakeStatusBarView != null) {
            ...
            decorView.addView(createStatusBarViewWithDrawable(activity, resId));
        }
        setRootView(activity);
    }
     ...
     */
    @SuppressLint("ResourceType")
    private static View createStatusBarViewWithDrawable(Activity activity, @IdRes int resId) {
        // 绘制一个和状态栏一样高的矩形
        View statusBarView = new View(activity);
        LinearLayout.LayoutParams params =
                new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getStatusBarHeight(activity));
