...
public final class L {
    ...
    private static final int XML = 0xF3;
    ...
    public static class Builder {
        ...
        //例如： L.Builder builder = new L.Builder("sunway/monit2/");
        public Builder(String path) {
            if (defaultDir != null) return;
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
                if (!StringUtils.isEmpty(path)) {
                    defaultDir = SDCardUtils.getSDCardPath() + path + "log" + File.separator;
                } else {
                    File baseCache = Utils.getContext().getExternalCacheDir();
                    if (baseCache == null) return;
                    defaultDir = baseCache.getPath() + File.separator + "log" + File.separator;
                }
            } else {
        ...
        /**
         * 设置所有log日志是否需要保存的文件中，否则请调用L.file(...);
         *
         * @param log2FileSwitch 默认为false
         **/
