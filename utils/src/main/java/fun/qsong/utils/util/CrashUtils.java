...
public final class CrashUtils
    ...
    /**
     * 获取单例
     * <p>在Application中初始化{@code Utils.init(this);CrashUtils.getInstance().init();}</p>
     ...
     * 初始化
     *
     * @param path 日志输出文件路径，例sunway/monit/
     * @return {@code true}: 成功<br>{@code false}: 失败
     */
    public boolean init(String path) {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            if (!StringUtils.isEmpty(path)) {
                crashDir = SDCardUtils.getSDCardPath() + path + "crash" + File.separator;
            } else {
                File baseCache = Utils.getContext().getExternalCacheDir();
                if (baseCache == null) return false;
                crashDir = baseCache.getPath() + File.separator + "crash" + File.separator;
    ...
    public void uncaughtException(Thread thread, final Throwable throwable) {
        ...
        if (throwable == null || !createOrExistsFile(fullPath)) return;
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if ("0.000B".equals(FileUtils.getFileSize(fullPath))) {
                    FileUtils.deleteFile(fullPath);
                }
            }
        }).start();
        if (mHandler != null && thread != null) {
    ...
    public void uncaughtException(String tag, final Throwable throwable) {
        if (TextUtils.isEmpty(crashDir)) {
            return;
        }
        String now = new SimpleDateFormat("yyyy-MM-dd HH-mm-ss", Locale.getDefault()).format(new Date());
        final String fullPath = crashDir + now + (TextUtils.isEmpty(tag) ? "" : " " + tag) + ".txt";
        if (throwable == null || !createOrExistsFile(fullPath)) return;
        new Thread(new Runnable() {
            @Override
            public void run() {
